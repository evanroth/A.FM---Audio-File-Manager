...
/**
 * Enhanced MIME type detection with broader compatibility.
 */
...
const App: React.FC = () => {
  ...
  const refreshFiles = useCallback(async () => {
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        metadataQueueRef.current = [...files];
      }
    ...
    const enrichMetadata = async () => {
      ...
      while (metadataQueueRef.current.length > 0) {
        ...
        try {
          ...
          // ignore error per-file during enrich
        }

        const now = performance.now();
        if (batchSize >= 50 || (now - lastBatchTime > 250 && batchSize > 0) || metadataQueueRef.current.length === 0) {
          const currentBatch = { ...batch };
          const currentMax = lastMaxDuration;
          
          setDurations(prev => ({ ...prev, ...currentBatch }));
          
          if (currentMax > bounds.duration.max) {
             setBounds(prev => ({ 
               ...prev, 
               duration: { ...prev.duration, max: Math.max(prev.duration.max, currentMax) } 
             }));
             setMaxDuration(v => v === bounds.duration.max ? currentMax : v);
          }
  ...
  const filteredFiles = useMemo(() => {
    return sortedFiles.filter(file => {
      ...
      const matchSearch = matchSmartSearch(file.name, searchText);
  ...
  }, [sortedFiles, durations, ratings, searchText, minSize, maxSize, minDuration, maxDuration, minDate, maxDate, minRating]);
  ...
  const playFile = useCallback(async (item: FileSystemItem, shouldPlay: boolean = true) => {
    ...
    try {
      ...
      if (audioRef.current) {
        ...
        audioRef.current.src = ""; // Clear existing
        audioRef.current.load();
        
        await new Promise(r => setTimeout(r, 10)); // Give browser a moment
        ...
        if (shouldPlay) {
          try {
            ...
            if (playErr.name !== 'AbortError') {
               ...
               setError(`Source Error: ${item.name}`);
      ...
      const audioBuffer = await audioCtxRef.current!.decodeAudioData(arrayBuffer).catch(e => {
        console.warn("Visualizer decode failed for:", item.name, e);
        return null; // Don't crash, just no waveform
      });
      ...
      if (requestId === playRequestCount.current) {
        console.error("App logic error during playFile:", e);
        setWaveformData(null);
      }
    }
    ...
    try {
      for (const source of itemsToMove) {
        ...
        if (source.parentHandle) {
          try {
            await source.parentHandle.removeEntry(source.name);
          } catch (removeErr) {
            console.warn("Could not remove source file after copy:", removeErr);
          }
        }
        movedCount++;
      }
      ...
      await refreshFiles();
      setError(`MOVED ${movedCount} FILE${movedCount > 1 ? 'S' : ''} -> /${target.name}`);
      setTimeout(() => setError(null), 2500);
    } catch (e) {
      console.error("Batch move failed:", e);
      setError("BATCH MOVE FAILED (LOCKED ENTRY?)");
      setTimeout(() => setError(null), 2500);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDirectoryPicker = async () => {
    try {
      ...
      if (e.name !== 'AbortError') {
        console.warn("Picker failed, using fallback:", e);
        fallbackInputRef.current?.click();
      }
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (['0', '1', '2', '3', '4', '5'].includes(e.key) && activeFileId) {
        e.preventDefault();
        handleRate(activeFileId, parseInt(e.key));
        return;
      }
  ...
  const addMoveShortcut = () => {
    ...
    setMoveShortcuts(prev => [...prev, { key: nextKey, targetPath: availableFolders[0]?.id || '' }]);
  };

  const updateShortcutTarget = (key: string, targetPath: string) => {
    setMoveShortcuts(prev => prev.map(s => s.key === key ? { ...s, targetPath } : s));
  };

  const removeShortcut = (key: string) => {
    setMoveShortcuts(prev => prev.filter(s => s.key !== key));
  };
  ...
  return (
    <div className="relative w-screen h-screen bg-black overflow-hidden flex text-white font-sans">
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="w-[40%] flex flex-col border-r border-white/5 overflow-y-auto scrollbar-hide">
                ...
                <div className="flex flex-nowrap gap-1">
                  {(['name', 'size', 'type', 'date', 'duration', 'rating'] as SortKey[]).map(key => (
                    ...
                    >
                        ...
                        {sortKey === key && (
                          <span className="text-[8px] leading-none shrink-0">{sortOrder === 'asc' ? '↑' : '↓'}</span>
                        )}
                    </button>
                  ))}
                </div>
            ...
            <div className="p-8 space-y-10">
              {/* Move Shortcut Configuration Section */}
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-neutral-400">Display File Name</span>
                  <button onClick={() => setShowFileInfo(!showFileInfo)} className={`h-4 w-8 relative transition-colors ${showFileInfo ? 'bg-red-600' : 'bg-neutral-800'}`}>
                    <span className={`absolute top-0.5 h-2 w-2 bg-white transition-transform ${showFileInfo ? 'left-5' : 'left-1'}`} />
                  </button>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-neutral-400">Color Waveform</span>
                  <button onClick={() => setIsColorWaveform(!isColorWaveform)} className={`h-4 w-8 relative transition-colors ${isColorWaveform ? 'bg-red-600' : 'bg-neutral-800'}`}>
                    <span className={`absolute top-0.5 h-2 w-2 bg-white transition-transform ${isColorWaveform ? 'left-5' : 'left-1'}`} />
                  </button>
                </div>

                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-neutral-400">Gradient Background</span>
                  <button onClick={() => setIsGradientBackground(!isGradientBackground)} className={`h-4 w-8 relative transition-colors ${isGradientBackground ? 'bg-red-600' : 'bg-neutral-800'}`}>
                    <span className={`absolute top-0.5 h-2 w-2 bg-white transition-transform ${isGradientBackground ? 'left-5' : 'left-1'}`} />
                  </button>
                </div>

                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-neutral-400">Gradient Waveform</span>
                  <button onClick={() => setIsGradientWaveform(!isGradientWaveform)} className={`h-4 w-8 relative transition-colors ${isGradientWaveform ? 'bg-red-600' : 'bg-neutral-800'}`}>
                    <span className={`absolute top-0.5 h-2 w-2 bg-white transition-transform ${isGradientWaveform ? 'left-5' : 'left-1'}`} />
                  </button>
                </div>
                
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-neutral-400">Shuffle</span>
                  <button onClick={() => setIsRandom(!isRandom)} className={`h-4 w-8 relative transition-colors ${isRandom ? 'bg-red-600' : 'bg-neutral-800'}`}>
                    <span className={`absolute top-0.5 h-2 w-2 bg-white transition-transform ${isRandom ? 'left-5' : 'left-1'}`} />
                  </button>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-neutral-400">Loop</span>
                  <button onClick={() => setIsLooping(!isLooping)} className={`h-4 w-8 relative transition-colors ${isLooping ? 'bg-red-600' : 'bg-neutral-800'}`}>
                    <span className={`absolute top-0.5 h-2 w-2 bg-white transition-transform ${isLooping ? 'left-5' : 'left-1'}`} />
                  </button>
                </div>
              </div>
          ...
          <div className="flex-1 flex flex-col bg-neutral-900/10 overflow-y-auto scrollbar-hide">
      ...
      {/* Fallback input for standard folder upload if API isn't available */}
      <input 
        {...({
